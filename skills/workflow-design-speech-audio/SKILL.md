...
- Review perspectives below are competencies, never real or virtual agents. The parent maps required perspectives to real runtime owners.
...
- Sources need authority, `observed_at`, effective version/date, exact locale/channel/purpose applicability, conflicts, and `valid_until` or recheck trigger. Generic benchmarks and vendor claims do not prove deployment fit.
- Unknown action, locales, channels, consent/voice rights, harm tolerance, acceptance thresholds, latency budget, handoff capacity, vendor retention, schedule, or rollout scope cannot authorize a commitment or be invented.
...
Return marginal deltas to v4. Barriers precede raw-audio access/transfer, enrollment or identity inference, external calls, voice publication, live thresholds, and rollout. Emit parent feedback for unexpected person/bystander audio, locale/channel outside frozen coverage, critical entity/negation error, speaker swap, latency/revision/barge-in breach, replay/synthetic signal, or consent/voice-right mismatch. Use authorized direct evidence and map safe stop plus affected gates/permits. A novel acoustic, semantic, or voice strategy is only an `InsightCandidate`; falsify on authorized end-to-end semantic/identity outcomes and untouched locale/channel confirmation, never clean-room WER alone.
...
DomainGatePacket:
  ...
  knowledge_sources: [source, authority, effective_version, observed_at, locale_channel_purpose, conflicts, valid_until, status]
  ...
  simplification_triggers: [gate -> positive not_applicable evidence and residual limits]
```
...
The parent must evaluate each applicable gate as `pass|fail|escalate` against named evidence refs; prose that merely mentions a concern is not proof. Every `fail|escalate` record must list the protected action it blocks, the owner of the missing evidence, and the reopen trigger. `N/A` requires positive applicability evidence and a residual-limit statement. Do not infer intelligibility, identity safety, or conversational readiness from clean-room audio alone.

## Self-Improvement Hook

After a task, emit a `DomainSkillUpdateCandidate` only when evidence shows a recurring missing condition, gate, perspective, or failure mode. Include task evidence and one failing-then-passing behavior test. Do not mutate the installed skill unless the user requested or approved improvement.
